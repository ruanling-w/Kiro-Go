...
const initial: ComboInput = { name: '', strategy: 'fallback', stickyLimit: 1, models: [{ model: '' }], fusionQuorum: 1, fusionTimeoutMs: 30000, judgeModel: '' }
function validate(v: ComboInput): ComboFieldErrors {
  ...
  const clean = v.models.map((candidate) => ({ model: candidate.model.trim(), provider: candidate.provider?.trim().toLowerCase() ?? '' }))
  if (clean.some((candidate) => !candidate.model) || new Set(clean.map((candidate) => `${candidate.provider}\0${candidate.model.toLowerCase()}`)).size !== clean.length) e.models = 'combos.validation.models'
...
export function ComboFormDialog({ open, onOpenChange, editing }: { open: boolean; onOpenChange: (open: boolean) => void; editing: Combo | null }) {
  ...
  useEffect(() => {
    if (!open) return
    setErrors({})
    setValue(editing ? { name: editing.name, strategy: editing.strategy, stickyLimit: editing.stickyLimit, revision: editing.revision, models: editing.models.map(({ model, provider }) => ({ model, provider })), fusionQuorum: editing.fusionQuorum || 1, fusionTimeoutMs: editing.fusionTimeoutMs || 30000, judgeModel: editing.judgeModel || '', judgeProvider: editing.judgeProvider || '' } : initial)
  }, [open, editing])
  ...
  function submit(event: React.FormEvent) {
    ...
    const body: ComboInput = { ...value, name: value.name.trim(), models: value.models.map(({ model, provider }) => ({ model: model.trim(), ...(provider?.trim() ? { provider: provider.trim().toLowerCase() } : {}) })) }
    if (body.strategy !== 'fusion') { delete body.fusionQuorum; delete body.fusionTimeoutMs; delete body.judgeModel; delete body.judgeProvider }
