...
export function LoginPage() {
  ...
  return (
    <div className="relative flex min-h-dvh items-center justify-center overflow-hidden bg-background px-4">
      {/* Ambient teal field — static radial washes behind the card. */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 -z-10 opacity-70 dark:opacity-50"
        style={{
          background:
            'radial-gradient(55% 45% at 20% 15%, var(--glow), transparent 70%), radial-gradient(50% 45% at 85% 90%, oklch(0.66 0.13 220 / 28%), transparent 70%)',
        }}
      />
      ...
      <div className="w-full max-w-sm rounded-2xl border bg-card/85 p-8 shadow-[0_24px_70px_-28px_var(--glow)] backdrop-blur-md ring-1 ring-primary/10">
        ...
        {isInsecureContext() && (
          <div className="mb-5 flex items-start gap-2 rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-xs text-amber-700 dark:text-amber-400">
